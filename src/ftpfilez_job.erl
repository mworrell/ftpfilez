...
start_link(JobId, Cmd) ->
    gen_server:start_link({via, gproc, {n,l,{ftpfilez_job,JobId}}}, ?MODULE, [JobId, Cmd], []).
...
do_cmd(JobId, {delete, Config, Url, OnReady}) ->
    Result = ftpfilez:delete(Config, Url),
    on_ready(JobId, Result, OnReady);
do_cmd(JobId, {put, Config, Url, What, OnReady, Options}) ->
    Result = ftpfilez:put(Config, Url, What, Options),
    on_ready(JobId, Result, OnReady);
do_cmd(JobId, {stream, Config, Url, Pid}) when is_pid(Pid) ->
    ftpfilez:stream(Config, Url, {JobId, Pid});
do_cmd(_JobId, {stream, Config, Url, StreamFun}) ->
    ftpfilez:stream(Config, Url, StreamFun).
